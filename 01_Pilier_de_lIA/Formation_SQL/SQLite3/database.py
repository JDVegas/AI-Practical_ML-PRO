...
# Create a connexion to a database
...
def insert_data():
    ...
    # Validate modifications
    conn.commit()
    print("Data successfuly incerted")
...
# -- x --------------------x --
# Select data from the table "AUTHEUR"
cursor.execute('''
    SELECT * FROM Auteurs
''')

# Extract the data from the selection into a list
authors = cursor.fetchall() # [(1, 'Hugo', 'Victor', 'France'), ...]

# Display the extracted data list
print("Author's list:")
# Iterate through each elements (tuple)
for author in authors:
    print(author) # (1, 'Hugo', 'Victor', 'France')
# -- x --------------------x --
...
# Select a book
cursor.execute('''
    SELECT Titre, AuteurID FROM Livres WHERE AuteurID = 1
''')

# Extract the data
book = cursor.fetchone() # ('Les Misérables', 1)

# IF .. the  book exists then dispay data
if book :
	print(f"Title: {book[0]}, AuthorID: {book[1]}")
# ELSE .. the book does not exist, then return mesage
else :
	print("No Book Found !")
# -- x --------------------x --
...
print("The connection have been properly closed !")